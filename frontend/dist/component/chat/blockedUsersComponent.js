...
// Component
// =============================================================================
function blockedUsersComponent(backLabel) {
    ...
    `);
        ...
        backButton.addEventListener('click', () => __awaiter(this, void 0, void 0, function* () {
            ...
            const peopleChats = yield currentPeopleComponent();
        ...
        // =============================================================================
        const rawUsers = yield api.fetchBlockedUsers();
        const users = rawUsers !== null && rawUsers !== void 0 ? rawUsers : [];
  ...
  <path d="M21.18,24.54A3.89,3.89,0,1,1,13.40,24.54A3.89,3.89,0,1,1,21.18,24.54Z"/>
        ...
        `);
            // --- Query Selectors ---
            const unblockButton = row.querySelector(`[data-id="unblock-${user.id}"]`);
            const avatarCell = row.querySelector('[data-id="avatar-cell"]');
            avatarCell.prepend(avatar);
            // --- Event Listeners ---
            unblockButton === null || unblockButton === void 0 ? void 0 : unblockButton.addEventListener('click', () => __awaiter(this, void 0, void 0, function* () {
                const confirmed = confirm(`Unblock ${user.username}?`);
                if (confirmed) {
                    const success = yield api.unblockUser(user.id);
                    if (success)
                        row.remove();
                }
            }));

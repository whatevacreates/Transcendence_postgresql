...
function rankingComponent() {
    ...
    `);
        // --- Selectors ---
        const rankingSelector = container.querySelector('[data-id="ranking-body"]');
        const users = yield api.fetchUsers();
        // Fetch stats and pair with users
        const userStatsPairs = yield Promise.all(users.map((user) => __awaiter(this, void 0, void 0, function* () {
        ...
        `);
            const userCell = row.querySelector('[data-id="user-cell"]');
